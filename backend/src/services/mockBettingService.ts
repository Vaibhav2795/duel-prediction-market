...
class MockBettingService {
		...
		if (filters?.search) {
			...
			filtered = filtered.filter(
				m =>
					...
					m.playerWhite?.displayName
						?.toLowerCase()
						.includes(searchLower) ||
					m.playerBlack?.displayName
						?.toLowerCase()
						.includes(searchLower)
			);
		}
	...
	// Get user positions
	async getUserPositions(): Promise<Position[]> { // eslint-disable-line @typescript-eslint/no-unused-vars
		await delay(50 + Math.random() * 100);
	...
	async placeBet(
		...
		const bet = placeMockBet(marketId, userAddress, outcome, amount);
		...
		return {
			success: false,
			error: "Failed to place bet. Market may be inactive or invalid."
		};
	}
	...
	simulateProbabilityUpdate(marketId: string): Market | null {
		...
		market.outcomes.forEach(outcome => {
			...
			outcome.probability = Math.min(
				95,
				Math.max(5, outcome.probability + adjustment)
			);
		...
		const total = market.outcomes.reduce(
