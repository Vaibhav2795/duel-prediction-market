...
import { encodeFunctionData, createPublicClient, http, type Address, parseUnits } from 'viem';
import {
  ...
  const betOnMatch = useCallback(
    async (matchId: number, outcome: 1 | 2 | 3, amount: string) => {
      ...
      if (outcome !== OUTCOME_PLAYER1 && outcome !== OUTCOME_PLAYER2 && outcome !== OUTCOME_DRAW) {
        throw new Error('Invalid outcome. Must be 1 (Player1), 2 (Player2), or 3 (Draw)');
      }
...
export function useGetMarketDataOnchain() {
  ...
  const getMarketData = useCallback(async (matchId: number) => {
    ...
    try {
      ...
      const [status, winningOutcome, totalPool, p1Shares, p2Shares, drawShares] = result;
